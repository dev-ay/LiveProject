...
## OVERVIEW
This is an Agile/SCRUM live project that I participated in at the Tech Academy for a production clock-in software.  The overall project occurs over many months, but my personal participation was over a single 2-week sprint.  This Readme only lists the stories assigned to me (albeit not a complete list).
...
### **Project Format:**
This was a multiple-month SCRUM project.  At the time of my participation, there were around 15 members on the team, many of them remote.  We held daily stand-up meetings over Google Hangout, and occassional story-building sessions to discuss and brainstorm overall product features and design.  Team members communicated and collaborated through Slack.  We used Microsoft's Azure DevOps online project management system to track each work item/story from creation through closure (i.e. "New", "Active", "Resolved", and "Closed"). During the first half of my sprint we used Microsoft's Team Foundation Server version control system to make commits and merge every team member's work.  During the second half we migrated over to Git version control instead.  
...
### Solution: 
The background for this story is that the edit modal, or *"SaveModal"*, would close even if a save attempt was unsuccessful, such as when entering invalid times.  The reason for this was that the `HTML` tag for the save button included the attribute 'data-dismiss="modal"', which in effect closes the edit modal each time the save button is clicked.  The solution therefore is to remove the "data-dismiss" attribute from the save button tag.

Along the same vein, the delete button also contains the same "data-dismiss" attribute.  This means that even if the user selects "No" when asked to confim a delete attempt, the edit modal would close.  Therefore, I also remove the "data-dismiss" attribute from the delete button tag. 
...
### Solution: 
When the calendar is first generated, the client requests events data from the server.  Among the information obtained during that process is whether the requested time-off events have been approved by an administrator.  Unapproved events are rendered in grey/silver, and approved events are rendered in green.  My motivation for this story/work item is the idea that events can be approved by an administrator at any time.  Therefore, there needs to be a way to update the approval status of events after the calendar has been generated, without having to reload all events data from the database repeatedly.
...
The solution I chose is to update an event's approval status when a user clicks on the event in the calendar, that is, if the event is not already approvded.  In other words, when the user clicks on an unapproved event, I make an AJAX call for the latest approval status for that event.  An additional feature I decided to add is a section for approval status on the "DetailsModal" with an **id** of "modalApproval".
...
In the initializer for the calendar I update the **eventClick** callback to display the current approval status of an event on the "DetailsModal" when it is displayed.  Then, if the event is not yet approved, I call the function "GetApproval" to check the server for an update.
```javascript
...
For the "GetApproval" function, I make an AJAX call to the backend for the latest approval status for the event currently being viewed.  If the status becomes approved, I update the "modalApproval" text in the "DetailsModal" and I also update the event color on the calendar for the corresponding event.
```javascript
